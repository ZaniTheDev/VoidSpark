...
export function DesignStep({ form, updateField, toggleMood, onNext }) {
  ...
  // Function to get missing fields message
  const getMissingFieldsMessage = () => {
    const missing = [];
    if (!form.paket) missing.push("paket");
    if (!form.style) missing.push("gaya visual");
    if (!form.palette) missing.push("palet warna");

    if (missing.length === 0) return "";
    if (missing.length === 1) return `Lengkapi dulu: ${missing[0]}`;
    if (missing.length === 2) return `Lengkapi dulu: ${missing.join(" dan ")}`;
    return `Lengkapi dulu: ${missing.slice(0, -1).join(", ")} dan ${missing.slice(-1)}`;
  };
  ...
  return (
    <TooltipProvider delayDuration={300}>
      <>
        ...
        {/* Wrapped NavButtons with Tooltip for disabled state */}
        {!canProceed ? (
          <Tooltip>
            ...
            <TooltipContent
              side="top"
              className="bg-black text-white border-none text-sm px-4 py-2 rounded-lg"
            >
